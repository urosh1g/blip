#include <stdio.h>

#include <logger/logger.h>
#include <shader.h>

const char* file_load_text(const char* filename) {
    FILE* f = fopen(filename, "r");
    if (!f) {
        log_error("Could not open %s", filename);
        exit(EXIT_FAILURE);
    }

    fseek(f, 0, SEEK_END);
    size_t filesize = ftell(f);

    char* text = malloc(filesize + 1);

    fseek(f, 0, SEEK_SET);
    fread(text, 1, filesize, f);
    text[filesize] = '\0';
    return text;
}

GLuint shader_load(const char* filename, GLenum shader_type) {
    const char* shader_code = file_load_text(filename);

    GLuint shader_id = glCreateShader(shader_type);
    if (!shader_id) {
        log_error("Could not create shader");
        return -1;
    }
    glShaderSource(shader_id, 1, &shader_code, NULL);
    GLenum err = glGetError();
    if (err != GL_NO_ERROR) {
        log_error("Couldn't set shader source, error code: %d", err);
    }
    glCompileShader(shader_id);
    free((char*)shader_code);
    GLint success;
    glGetShaderiv(shader_id, GL_COMPILE_STATUS, &success);
    if (success == GL_FALSE) {
        GLchar* log = malloc(1024);
        glGetShaderInfoLog(shader_id, 1024, NULL, log);
        log_error("Couldn't compile shader %s. %s", filename, log);
        free(log);
        glDeleteShader(shader_id);
        exit(EXIT_FAILURE);
    }
    return shader_id;
}

void shader_destroy(GLuint shader_id) { glDeleteShader(shader_id); }

void program_use(GLuint program_id) { glUseProgram(program_id); }

GLuint program_link(GLuint* shaders, size_t num) {
    GLuint program_id = glCreateProgram();
    for (size_t i = 0; i < num; i++) {
        glAttachShader(program_id, shaders[i]);
    }
    glLinkProgram(program_id);
    GLint err;
    glGetProgramiv(program_id, GL_LINK_STATUS, &err);
    if (err != GL_TRUE) {
        log_error("Program didn't link successfully.");
        GLchar* log = malloc(1024);
        glGetProgramInfoLog(program_id, 1024, NULL, log);
        log_error("Could not link program: %s", log);
        free(log);
        exit(EXIT_FAILURE);
    }
    for (size_t i = 0; i < num; i++) {
        glDetachShader(program_id, shaders[i]);
    }
    return program_id;
}
