...
def fetch_uri_data(contents,gltf_filename):
    ...
    if uri.startswith("data:"):
        ...
        bin_path=os.path.join(os.path.dirname(gltf_filename),uri)
...
def fetch_indices(contents, endianness):
    ...
    if not "indices" in meshes[0]["primitives"][0]:
        print("no indices fount")
        return -1
...
binary=fetch_uri_data(contents,filename)
