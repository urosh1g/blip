...
static geometry_data_t* normal_load(glb_t* glb, gltfprimitive_t* primitive,
    ...
    uint32_t* accessor_pos =
        htable_attributes_get(&primitive->attributes, "NORMAL");
    if (!accessor_pos) {
    ...
    bufferView_t* normals_buffView =
        dynarr_bufferView_get(bufferViews, normals_buffView_ind);
...
static geometry_data_t* indices_load(glb_t* glb, gltfprimitive_t* primitive,
                                     ...
                                     dynarr_bufferView_t* bufferViews) {
    uint32_t* accessor_pos =
        htable_attributes_get(&primitive->attributes, "indices");
    if (!accessor_pos) {
    ...
    while (index < indices_count) {
        ...
        memcpy(&((char*)indices->data)[index * size], &buff[offset],
               component_size);
        // log_info("GL_COMPTYPE=%d offset:%d, i%d=%d",component_type,offset,
        // index, ((uint16_t*)indices->data)[index]);
    ...
    for (size_t i = 0; i < meshes->length; i++) {
        ...
        for (size_t j = 0; j < meshes->elems[i].primitives.length; j++) {
            geometry_data_t* vertices =
                position_load(glb, &meshes->elems[i].primitives.elems[j],
...
void primitive_draw(uint32_t VAO, primitive_t* p) {
    ...
    if (p->indices)
        glDrawElements(p->rendermode, p->indices->count,
                       p->indices->GL_component_type, 0);
    else
        glDrawArrays(p->rendermode, 0, p->vertices->count);
    glBindVertexArray(0);
}

void gltfnode_draw(gltfnode_t* root, mat4 model, uint32_t model_uniform_id,
    ...
    if (root->mesh) {
        ...
        for (size_t j = 0; j < primitives_count; j++) {
            ...
            glUniformMatrix4fv(model_uniform_id, 1, GL_FALSE,
                               (const float*)model);
        ...
        for (size_t j = 0; j < primitive_count; j++) {
            ...
            glBufferData(GL_ARRAY_BUFFER,
                         primitive->vertices->count *
                             primitive->vertices->component_size *
                             primitive->vertices->component_type,
                         primitive->vertices->data, GL_STATIC_DRAW);
            glVertexAttribPointer(0, primitive->vertices->component_size,
            ...
            if (primitive->indices) {
                ...
                glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                             primitive->indices->count *
                                 primitive->indices->component_size *
